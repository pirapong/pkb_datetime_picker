...
import 'package:pkb_datetime_picker/src/date_format.dart';
import 'package:pkb_datetime_picker/src/i18n_model.dart';
...
class DatePickerModel extends CommonPickerModel {
  ...
  DatePickerModel(
    ...
    this.buddhistCalendar = buddhistCalendar ?? true;
  ...
  void _fillLeftLists() {
    this.leftList = List.generate(maxTime.year - minTime.year + 1, (int index) {
      // print('LEFT LIST... ${minTime.year + index + (buddhistCalendar == true ? 543 : 0)}${_localeYear()}');
      return '${minTime.year + (buddhistCalendar == true ? 543 : 0) + index}${_localeYear()}';
    });
  }
  ...
  @override
  DateTime finalTime() {
    // print(currentTime.month.toString().length);
    // return buddhistCalendar == true
    //     ? DateTime.parse((currentTime.year + 543).toString() +
    //         '-' +
    //         (currentTime.month.toString().length == 1
    //             ? '0' + currentTime.month.toString()
    //             : currentTime.month.toString()) +
    //         '-' +
    //         currentTime.day.toString())
    //     : DateTime.parse(currentTime.year.toString() +
    //         '-' +
    //         (currentTime.month.toString().length == 1
    //             ? '0' + currentTime.month.toString()
    //             : currentTime.month.toString()) +
